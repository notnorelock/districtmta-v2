...
local function createElementMatrix(pos, rot)
  ...
  return {
    { mathCos(rz) * mathCos(ry) - mathSin(rz) * mathSin(rx) * mathSin(ry), mathCos(ry) * mathSin(rz) + mathCos(rz) * mathSin(rx) * mathSin(ry), -mathCos(rx) * mathSin(ry), 0 },
    { -mathCos(rx) * mathSin(rz),                                          mathCos(rz) * mathCos(rx),                                           mathSin(rx),                0 },
    { mathCos(rz) * mathSin(ry) + mathCos(ry) * mathSin(rz) * mathSin(rx), mathSin(rz) * mathSin(ry) - mathCos(rz) * mathCos(ry) * mathSin(rx), mathCos(rx) * mathCos(ry),  0 },
    { pos[1],                                                              pos[2],                                                              pos[3],                     1 }
  }
end
...
function drawTransformedMaterial(texture, posX, posY, posZ, dirX, dirY, dirZ, sizeX, sizeY, color, offsetX, offsetY, offsetZ)
  ...
  -- Draw front side
  dxDrawMaterialLine3D(v1x, v1y, v1z, v2x, v2y, v2z, false, texture, sizeX, tocolor(color[1], color[2], color[3], color[4]), false, vUx, vUy, vUz)
end
