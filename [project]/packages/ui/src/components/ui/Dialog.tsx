...
// data-expanded:animate-in/fade-in/zoom-in-95 etc. require tw-animate-css (globals.css) -
// without it these classes silently no-op instead of erroring.
...
const DialogPortal: Component<DialogPrimitive.DialogPortalProps> = (props) => {
  ...
  return (
    <DialogPrimitive.Portal {...rest}>
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4">{props.children}</div>
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        class={cn(
          "relative z-50 grid max-h-screen w-full max-w-sm gap-4 overflow-y-auto rounded-lg border border-border bg-card p-6 text-card-foreground shadow-2xl duration-200 data-expanded:animate-in data-closed:animate-out data-closed:fade-out-0 data-expanded:fade-in-0 data-closed:zoom-out-95 data-expanded:zoom-in-95",
          props.class,
        )}
        {...rest}
      >
...
const DialogTitle = <T extends ValidComponent = "h2">(props: PolymorphicProps<T, DialogTitleProps<T>>) => {
  ...
  return <DialogPrimitive.Title class={cn("text-lg font-semibold leading-none tracking-tight", props.class)} {...rest} />;
};
