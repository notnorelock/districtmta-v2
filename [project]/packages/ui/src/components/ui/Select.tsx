  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          "relative z-50 min-w-32 overflow-hidden rounded-md border border-border bg-popover text-popover-foreground shadow-md data-expanded:animate-in data-expanded:fade-in-80",
          local.class,
        )}
        {...others}
      >
...
const SelectLabel = <T extends ValidComponent = "label">(props: PolymorphicProps<T, SelectLabelProps<T>>) => {
  ...
  return <SelectPrimitive.Label class={cn(selectLabelVariants(), local.class)} {...others} />;
};
...
const SelectDescription = <T extends ValidComponent = "div">(props: PolymorphicProps<T, SelectDescriptionProps<T>>) => {
  ...
  return <SelectPrimitive.Description class={cn(selectLabelVariants({ variant: "description" }), local.class)} {...others} />;
};
...
const SelectErrorMessage = <T extends ValidComponent = "div">(props: PolymorphicProps<T, SelectErrorMessageProps<T>>) => {
  const [local, others] = splitProps(props as SelectErrorMessageProps, ["class"]);
  return <SelectPrimitive.ErrorMessage class={cn(selectLabelVariants({ variant: "error" }), local.class)} {...others} />;
};
