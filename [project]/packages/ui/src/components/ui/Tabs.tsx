...
// Unlike upstream solid-ui, TabsContent has no unconditional base classes (mt-2, focus
// ring) - AuthCard.tsx fully overrides layout/animation via its own auth-panel__* classes.
...
const TabsList = <T extends ValidComponent = "div">(props: PolymorphicProps<T, TabsListProps<T>>) => {
  ...
  return (
    <TabsPrimitive.List
      class={cn("inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground", local.class)}
      {...others}
    />
  );
};
...
const TabsIndicator = <T extends ValidComponent = "div">(props: PolymorphicProps<T, TabsIndicatorProps<T>>) => {
  ...
  return (
    <TabsPrimitive.Indicator
      class={cn(
        "absolute transition-all duration-250 data-[orientation=horizontal]:-bottom-px data-[orientation=vertical]:-right-px data-[orientation=horizontal]:h-0.5 data-[orientation=vertical]:w-0.5",
        local.class,
      )}
      {...others}
    />
