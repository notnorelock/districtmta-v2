...
| `gm_vehicles` | `script`, server-only | Persistent PRIVATE vehicles (database-backed via `core`'s `Vehicle`/`VehicleRepository`, owner = `accounts.id` - no character-select system exists yet) and scripted PUBLIC vehicles (`PublicVehicles.lua` - a fixed, in-code spawn point list, never persisted). Since a callback can never cross a resource boundary (see "The one hard rule for extending the project" above), `VehicleBridge.lua` reaches `core`'s `VehicleRepository` through `core/server/database/VehicleService.lua`, a requestId-correlated event bridge mirroring `FetchBridge`'s own pattern (see "FetchBridge across the core/core_ui boundary" below) rather than a direct callback-bearing export. `/createvehicle <model>` (gated on `Permissions.Bit.SPAWN_VEHICLE`, MODERATOR+) spawns a new private vehicle owned by the issuer's own account. See `docs/DatabaseContract.md`'s "Vehicles table" section for the full column reference and what was deliberately NOT ported from the reference implementation this was built from (GROUP/EVENT/EXCHANGE/SHOP/RENT purposes, tuning workshops, the vehicle exchange, vehicle sharing - none of those systems exist in this project). Does NOT own the radial interaction menu - see `gm_vehicles_interaction` below, a deliberately separate resource. |
| `gm_vehicles_interaction` | `script`, server + client | The radial vehicle interaction menu (left Shift, held while driving, scroll to select, space to activate - CEF, see `VehicleMenuOverlay.tsx`) toggling engine/lights/lock/handbrake (`setElementFrozen`, not GTA's own space-bar handbrake - fully immobile). A separate resource from `gm_vehicles` on purpose: it only ever touches transient, already-synced MTA vehicle state, never the database, and works on ANY vehicle the local player is driving (`gm_vehicles`'s own persistent ones, `gm_roleplay`'s `/veh` temporary ones, or anything else) rather than depending on `gm_vehicles` at all. Exports `vehicleInteractionIsMenuOpen` (client) so `gm_radio`'s own scroll-to-change-station handling can skip itself while this menu is open - scroll wheel drives both, and without this check, moving the ring selection also cycled the radio station underneath it. |
...
`core_auth` → `core_admin` → `gm_voice` → `gm_vehicles` →
`gm_vehicles_interaction` → `gm_radio` → `gm_blackout` → `gm_scoreboard`
→ `ui_hud` → `gm_roleplay`, enforced automatically by `core_bootstrap`
