...
`core_auth` → `core_admin` → `gm_voice` → `gm_radio` → `gm_blackout` →
`gm_scoreboard` → `gm_vehicles` → `ui_hud` → `gm_roleplay`, enforced
automatically by `core_bootstrap`
